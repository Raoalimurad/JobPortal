...
const PostJob = () => {
    ...
    const handleJobPost = async () => {
        const data = { title: name, description, experience, requirments: requirements, Salary: salary, location, jobType, position, companyId }

        try {
            dispatch(setLoading(true))
            const response = await axios.post(`/api/job/registration`, data, { withCredentials: true })
                ...
                <div className='  md:w-[460px] lg:w-[560px] mx-auto '>
                    ...
                    </div>
                    {
                        loading ? <button className='btn flex items-center justify-center'>
                            ...
                            :
                            <button className='btn' onClick={handleJobPost}>Post </button>
                    }
